    ...
    for (const user of users) {
        try {
            ...
            await new Promise((resolve, reject) => {
                db.run(`INSERT OR IGNORE INTO users (name, password, role_id) VALUES (?, ?, ?)`, [user.name, hash, user.role_id], function(err) {
...
async function insertFollowers(followers) {
    for (const follower of followers) {
        await new Promise((resolve, reject) => {
            db.run(`INSERT OR IGNORE INTO followers (follower_id, followed_id) VALUES (?, ?)`, [follower.follower_id, follower.followed_id], function(err) {
                if (err) return reject(err);
                resolve();
            });
        });
...
async function insertRoles(roles) {
    for (const role of roles) {
        await new Promise((resolve, reject) => {
            db.run(`INSERT OR IGNORE INTO roles (role_name) VALUES (?)`, [role.role_name], function(err) {
                if (err) return reject(err);
                resolve();
            });
        });
...
async function insertPosts(posts) {
    for (const post of posts) {
        await new Promise((resolve, reject) => {
            db.run(`INSERT OR IGNORE INTO posts (content, user_id) VALUES (?, ?)`, [post.content, post.user_id], function(err) {
                if (err) return reject(err);
                resolve();
            });
        });
...
async function insertComments(comments) {
    for (const comment of comments) {
        await new Promise((resolve, reject) => {
            db.run(`INSERT OR IGNORE INTO comments (post_id, user_id, content) VALUES (?, ?, ?)`, [comment.post_id, comment.user_id, comment.content], function(err) {
                if (err) return reject(err);
                resolve();
            });
        });
